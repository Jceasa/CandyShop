# 🍬 PDV Web - Gerenciamento de Venda de Doces

> Projeto desenvolvido como Atividade Extensionista do curso de Análise e Desenvolvimento de Sistemas.

## 📋 Sobre o Projeto

.. **Em desenvolvimento**

## 🚀 Funcionalidades

**Em desenvolvimento**

## 🛠️ Tecnologias Utilizadas

**Em desenvolvimento**

## 📱 Layout e Engenharia

O projeto seguiu etapas de Engenharia de Software antes da codificação:
1.  **Levantamento de Requisitos:** Entrevista com a stakeholder (cliente).
2.  **Prototipagem:** Wireframes desenhados e validados.
3.  **Implementação:** Desenvolvimento focado em usabilidade móvel.

[FOTO DA TELA AQUI - PENDENTE]


## 🎨 Créditos e Atribuições

Este projeto utilizou recursos de terceiros e ferramentas de IA:

* **Ícone:** Sinal de Menos criado por **[iconsax]** via **[https://www.flaticon.com/br/]**.
* **Ícone:** Sinal de Mais criado por **[iconsax]** via **[https://www.flaticon.com/br/]**.
* **Imagens dos Produtos:** Geradas via Inteligência Artificial (**Google Gemini**).
* **Tratamento de Imagens:** Remoção de fundo realizada via **[https://www.remove.bg/pt-br]**.



## 🔗 Como Acessar

Você pode acessar o projeto rodando diretamente no seu navegador através deste link:
👉 **[CLIQUE AQUI PARA ABRIR O SISTEMA](LINK_DO_GITHUB_PAGES_AQUI)**

---
Desenvolvido por **[Júlio César]**
[LinkedIn](https://www.linkedin.com/in/jc-ceasar/) | [GitHub](https://github.com/Jceasa)